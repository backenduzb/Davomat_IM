...
def get_all_no():
    ...
    for class_name, info in result.items():
        ...
        result[class_name]["present_percent"] = (
            f"{total}ta o'quvchidan {absent}tasi kelmagan."
        )

    total_absent = total_reason + total_no_reason
    total_present_percent = (
        round(((total_students - total_absent) / total_students) * 100, 1)
        if total_students
        else 0
    )
