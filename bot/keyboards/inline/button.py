...
async def finish(inf: str) -> InlineKeyboardMarkup:
    button = [
        [InlineKeyboardButton(text="Sababsizlarni kirtish ➕" if inf=="no_reason" else "Sabablilarni kirtish ➕", callback_data=f"have_{inf}")],
        [InlineKeyboardButton(text="Sababsizlar yo'q ✅" if inf=="no_reason" else "Sabalilar yo'q ✅", callback_data=f"finish_{inf}")],
    ]
